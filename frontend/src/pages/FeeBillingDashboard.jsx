...
// Helper function to decode raw HTML entities like &quot;, &#39;, &amp;
const cleanText = (str) => {
...
function FeeBillingDashboard({
  ...
  const loadTickets = async () => {
    ...
    const endpoints = [
      ...
      `${API_BASE_URL}/tickets/desk/FEE_BILLING`,
      `${API_BASE_URL}/tickets`,
    ]

    let loadedTickets = null
    let lastError = 'Assigned tickets could not be loaded.'
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        if (response.ok) {
          ...
          if (result.length > 0 || loadedTickets === null) {
            loadedTickets = result
          }
          if (result.length > 0) break
        } else {
          const errorData = await response.json().catch(() => ({}))
          lastError = errorData.detail || `Server returned ${response.status}`
        }
      } catch (err) {
      ...
      <header className="bg-slate-900 text-white px-6 py-3.5 flex justify-between items-center shadow-md shrink-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] text-blue-400 font-medium">Accounts Desk • Fee & Billing</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right hidden sm:block">
            <div className="text-xs font-semibold text-slate-200">{profile?.full_name || 'Accounts Officer'}</div>
            <div className="flex items-center justify-end gap-1.5 mt-0.5">
              <span className="px-1.5 py-0.2 text-[8px] font-bold text-emerald-400 bg-emerald-950/80 rounded border border-emerald-800 uppercase">
      ...
      <main className="flex-1 p-4 md:p-6 overflow-y-auto max-w-7xl mx-auto w-full space-y-5">
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Fee & Billing Desk queue for <span className="font-semibold text-slate-700">{profile?.full_name || 'Verification Officer'}</span>
            </p>
          </div>
        ...
        {!loading && !errorMessage && tickets.length === 0 && (
          <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm">
            ...
            <h3 className="text-sm font-bold text-slate-800 mb-1">No Tickets Assigned</h3>
                  ...
                  <div className="lg:col-span-7 space-y-3">
                    <div>
                      <div className="flex items-center gap-2 text-xs text-slate-500 mb-1">
                        ...
                        <span>•</span>
                        <span>Category: <strong className="text-blue-600">{cleanText(ticket.category || 'Fee Verification & Billing')}</strong></span>
                      </div>
